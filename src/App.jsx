...
function FormFornecedor({initial,onSave,onCancel}){
  ...
  // Mínimo obrigatório: razão social + documento fiscal
  const canSave=(
    f.razaoSocial.trim()&&
...
function TelaFornecedores({fornecedores,onAdd,onEdit,onDelete}){
  ...
  return <div>
    ...
    {showForm&&<Modal title={editing?"Editar Fornecedor":"Cadastrar Fornecedor"} onClose={()=>setShowForm(false)} width={800}>
      <FormFornecedor initial={editing} onSave={async(forn)=>{
        try{editing?await onEdit(forn):await onAdd(forn);setShowForm(false);setEditing(null);}
